...
const STAGES: Stage[] = [
  ...
  },
  {
    ...
    what: "Discounts the projection — a new situation (offensive scheme, target competition, blocking, quarterback) is a real reset that last year's pace doesn't capture. 40% for RB, 70% for WR — RB and WR were swept separately and each has a real, found peak (the numbers rise then roll over smoothly on both sides of it in the backtest); WR's 70% looks steep, but it's the single largest, cleanest effect measured anywhere in this line of testing, not a guess. Backtested 2017–2025, measured two ways like the opportunity model above: against the pure model (QB/RB/WR all cleared the bar) and re-baselined against the live board — QB's gain nearly vanished there (already captured by QB's own injury discount and expert-blend weight), but RB and WR held up. Coaching changes, QB changes, team pace, and nuancing the discount by the QUALITY of the team a player joined were also tested as separate ideas and none of them cleared the bar, so only the flat RB/WR discount shipped. A player with no recorded team from last season (a rookie) is untouched.",
  },
  {
