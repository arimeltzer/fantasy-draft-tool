...
const STAGES: Stage[] = [
  ...
  },
  {
    ...
    what: "Discounts the projection — a new situation (offensive scheme, target competition, blocking, quarterback) is a real reset that last year's pace doesn't capture. 40% for RB, 25% for WR (RB and WR were swept separately; RB's discount turned out to have a real peak at 40%, WR's was still improving at the top of the range tested, so it stays at the more conservative, already-confirmed number rather than guessing higher). Backtested 2017–2025, measured two ways like the opportunity model above: against the pure model (QB/RB/WR all cleared the bar) and re-baselined against the live board — QB's gain nearly vanished there (already captured by QB's own injury discount and expert-blend weight), but RB and WR held up. Coaching changes, QB changes, team pace, and nuancing the discount by the QUALITY of the team a player joined were also tested as separate ideas and none of them cleared the bar, so only the flat RB/WR discount shipped. A player with no recorded team from last season (a rookie) is untouched.",
  },
  {
