...
export default function AuctionRoom({ league, settings, board, leagueId }: Props) {
  ...
  const ceilingFor = useCallback((p: BoardPlayer, market: number) => {
    ...
    // Diminishing RB/WR bench depth (roadmap 3.6f) was tried here and
    // REVERTED — a user's own roster-construction refinement ("a 5th and
    // down has diminishing returns, especially at the expense of a 3rd
    // WR/RB"), shipped ahead of its own kill gate (a real process gap,
    // caught on direct question), then measured retroactively: 3,600 paired
    // auctions scored on realized weekly points came back mean/SE -0.08
    // (calm) and -0.33 (early-overspend) vs the pre-3.6f baseline —
    // indistinguishable from noise in BOTH pre-registered buckets, nowhere
    // close to the >2 bar. `benchDepthMult` itself still lives in
    // budget-path.js (reused by the snake-side port under its own,
    // not-yet-shipped gate — docs/ROADMAP.md 3.6f-snake) and by
    // auction-sim.mjs's "treatment-depth" mode for any future re-test, but
    // it is no longer part of this ceiling. See docs/ROADMAP.md 3.6f.
